...
import { MessageSquare, Bot, Send, Loader2, Sparkles, Plus, Trash2 } from "lucide-react";
...
const SupportPage = () => {
  ...
  const saveMessage = useCallback(async (conversationId: string, role: string, content: string) => {
    await supabase.from("ai_support_messages").insert({ conversation_id: conversationId, role, content });
  }, []);
  ...
  const startNewChat = () => { setActiveConversationId(null); setMessages([]); };
  ...
  const sendMessage = async (text: string) => {
    ...
    if (!convId) {
      ...
      if (!convId) { toast.error("Не удалось создать чат"); setIsLoading(false); return; }
  ...
  const submitTicket = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!ticketSubject.trim() || !ticketMessage.trim() || !user || !companyId) return;
      ...
      <Tabs defaultValue="ai" className="w-full">
            ...
            <Card className="card-shadow w-64 shrink-0 flex-col hidden md:flex">
              ...
              <ScrollArea className="flex-1">
                <div className="p-2 space-y-1">
                  {conversations.map((conv) => (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{format(new Date(conv.updated_at), "dd.MM.yy HH:mm")}</p>
                      </div>
                  ...
                  {conversations.length === 0 && <p className="text-xs text-muted-foreground text-center py-4">Нет чатов</p>}
                </div>
              </ScrollArea>
            </Card>
                  ...
                  <div className="flex-1 overflow-x-auto">
                    <div className="flex gap-1">
                      {conversations.slice(0, 5).map((conv) => (
                        <Button key={conv.id} variant={activeConversationId === conv.id ? "default" : "ghost"}
                          size="sm" className="text-xs shrink-0 max-w-[120px]"
                          onClick={() => setActiveConversationId(conv.id)}>
                ...
                {messages.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-full text-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">Задайте вопрос о работе системы — я помогу разобраться</p>
                    </div>
                    <div className="flex flex-wrap gap-2 justify-center max-w-md">
                      {SUGGESTED_QUESTIONS.map((q) => (
                        <Button key={q} variant="outline" size="sm" className="text-xs h-auto py-2 px-3 whitespace-normal text-left"
              ...
              <div className="border-t p-3">
                <form onSubmit={(e) => { e.preventDefault(); sendMessage(input); }} className="flex gap-2">
                  <Input value={input} onChange={(e) => setInput(e.target.value)}
                    placeholder="Спросите что-нибудь о системе..." disabled={isLoading} className="flex-1" />
        ...
        <TabsContent value="ticket" className="mt-4 space-y-4">
          <Card className="p-6 card-shadow">
            ...
            <form className="mt-4 space-y-4" onSubmit={submitTicket}>
              <div>
        ...
        <TabsContent value="telegram" className="mt-4">
          <Card className="p-6 card-shadow">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-lg font-semibold">Написать в Telegram</h2>
                <p className="text-sm text-muted-foreground mt-1">Быстрая помощь от команды FILTER CRM</p>
              </div>
            </div>
            <Button className="mt-4" onClick={() => window.open("https://t.me/pzlllv", "_blank")}>
              Открыть Telegram — @pzlllv
            </Button>
          </Card>
        </TabsContent>
      </Tabs>
