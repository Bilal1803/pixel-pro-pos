...
import {
  Shield, Image, Building2, Users, CreditCard, LogOut,
  LayoutDashboard, HeadphonesIcon, MoreHorizontal,
  UserCog, Bell, AlertTriangle, ScrollText, Activity,
} from "lucide-react";
...
const navItems: NavItem[] = [
  ...
  { to: "/admin/stories", label: "Баннеры", icon: Image, roles: ["full_admin"] },
  { to: "/admin/notifications", label: "Уведомления", icon: Bell, roles: ["full_admin", "manager"] },
  { to: "/admin/admins", label: "Администраторы", icon: UserCog, roles: ["full_admin"] },
  { to: "/admin/errors", label: "Ошибки", icon: AlertTriangle, roles: ["full_admin"] },
  { to: "/admin/audit", label: "Журнал", icon: ScrollText, roles: ["full_admin"] },
  { to: "/admin/monitoring", label: "Мониторинг", icon: Activity, roles: ["full_admin"] },
];
    ...
    <>
      <Drawer open={drawerOpen} onOpenChange={setDrawerOpen}>
        <DrawerContent className="max-h-[80vh]">
          <div className="px-4 pt-2 pb-6 overflow-y-auto" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 2rem)" }}>
              ...
              {items.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex flex-col items-center gap-2 rounded-xl p-3 min-h-[68px] transition-all duration-150 active:scale-95",
            ...
            <button
              ...
              className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium text-destructive hover:bg-destructive/5 transition-colors"
            >
          ...
          {mainItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={cn("text-[10px] leading-tight font-medium", active && "font-semibold")}>{item.label}</span>
                {active && <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-6 h-0.5 rounded-full bg-destructive animate-scale-in" />}
              </button>
            );
          })}
          {moreItems.length > 0 && (
            ...
            >
              ...
              {isInMore && !drawerOpen && <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-6 h-0.5 rounded-full bg-destructive" />}
            </button>
          )}
        </div>
      </nav>
  ...
  return (
    <div className="flex min-h-screen">
      ...
      {!isMobile && (
        <aside className="flex h-screen w-60 flex-col border-r bg-card sticky top-0">
          <div className="flex h-14 items-center gap-2 border-b px-5">
            ...
            <span className="text-base font-bold">FILTER Admin</span>
          </div>
          {rl && (
            <div className="px-5 py-2 border-b">
              <Badge variant={rl.variant} className="text-[10px]">{rl.label}</Badge>
            </div>
          )}
      ...
      <div className="flex-1 min-w-0 overflow-y-auto">
        <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b bg-background/95 backdrop-blur px-4 md:px-6 gap-3">
          ...
          {rl && isMobile && (
            <Badge variant={rl.variant} className="text-[10px] shrink-0">{rl.label}</Badge>
          )}
        </header>
